...
Noted while checking whether the `api` policy grants anything real. **The table
that was here was wrong, and issue #1524 inherited the error** — it was read off
`/rest/ip/service` without separating static entries from dynamic ones, which
dropped the real services and kept things that are not `/ip/service` at all.
Corrected in Wave 12 by filtering on `dynamic == "false"`:

| Service | Port | Enabled | Note |
| --- | --- | --- | --- |
| `ftp` | 21 | **yes** | |
| `ssh` | 22 | yes | |
| `telnet` | 23 | **yes** | plaintext remote administration |
| `www` | 80 | **yes** | plaintext web administration |
| `www-ssl` | 443 | yes | `certificate: default-cert` |
| `winbox` | 8291 | yes | |
| `api` | 8728 | yes | **plaintext** binary API — granting the `api` policy is not inert |
| `api-ssl` | 8729 | yes | `certificate: none`, so it cannot actually serve TLS |

That is the whole list — exactly eight, `address` empty on every one.

**`telnet`, `ftp` and `www` are all enabled**, the opposite of what was recorded
here before.

The entries that *were* listed — `btest`, `discover`, `resolver` — are not
`/ip/service` rows. They appear in that REST view flagged `dynamic: true` and
live in their own subsystems: `/tool/bandwidth-server`,
`/ip/neighbor/discovery-settings`, and the DNS server. Wave 12 manages the first
two. `wireguard-wireguard1` on 13231 is dynamic for the same reason — it follows
the WireGuard interface, which Wave 6a manages.
...
### Wave 12 — inert system services *(done, partially)*

`services.tf`. Three of the five resources this wave aimed at imported clean;
234 resources total. The other two cannot be imported at all, which is the
finding.

| Resource | |
| --- | --- |
| `routeros_ip_settings` | imported |
| `routeros_ip_smb` | imported |
| `routeros_tool_bandwidth_server` | imported |
| `routeros_ip_service` | **cannot be imported** |
| `routeros_ip_neighbor_discovery_settings` | **cannot be imported** |

Singleton imports take a literal `.`, same as Wave 1.

#### The two that cannot be imported

Both are provider limitations at **v1.99.1, the newest release** — checked
against the registry, so there is no upgrade to wait for.

**`routeros_ip_service`** resolves the object and then reads back nothing:

```
Importing from ID "ftp"...
Refreshing state... [id=*1]        <- the provider DID find it
Error: Cannot import non-existent remote object
```

The lookup works — `ftp` resolves to `*1`, and both `/rest/ip/service/ftp` and
`/rest/ip/service/*1` return the object. The read is what comes back empty. The
likely cause is in the schema: `numbers` is required and non-computed while
`name` is computed, and the API returns `name`, not `numbers` — so nothing
populates the field the resource is addressed by, and tofu reads that as absent.

Whether a plain `apply` would work anyway is unknown: RouterOS cannot *add* an
`/ip/service` entry, so the provider's create path must be a `set` on an existing
one. That is plausible but unverified, and testing it means letting tofu create
against the management plane — winbox, api and ssh included. Not worth guessing
at. A bounded experiment is described in #1524.

**`routeros_ip_neighbor_discovery_settings`** fails earlier, with
`attribute .id not found in the response`. `/ip/neighbor/discovery-settings`
returns a bare settings object with no `.id` — but so do `/ip/settings` and
`/ip/smb`, and both import fine, so this is a per-resource defect rather than a
general one.

#### What is actually on the device

`rp-filter: no` and `tcp-syncookies: false` are RouterOS defaults, not local
choices. Neither is changed here; reverse-path filtering interacts badly with
the asymmetric paths BGP can produce, so it is not a free win.

`ip/smb` reports `enabled: auto` but `status: disabled` — auto means "run only
if a share exists", and none does, so nothing is listening. Worth switching off
outright rather than left on auto, but that is a change, not an import.

`/tool/bandwidth-server` is enabled with `authenticate: true` and 100 sessions.
Authentication makes it much less of a free amplification primitive than #1524
assumed, though nothing uses it.

### Wave 13 — the physical and WAN layer *(done)*

Appended to `network.tf`, with the per-port map in `locals.tf`. Six
`routeros_interface_ethernet` plus one `routeros_ip_dhcp_client`; all seven
imported clean on the first plan. **241 resources.**

```bash
just mikrotik import 'routeros_interface_ethernet.this["ether1"]' '*1'   # ... through *6 = sfp1
just mikrotik import routeros_ip_dhcp_client.wan '*1'
```

#### `factory_name` needs ignore_changes

The provider requires `factory_name` in config, but the API calls that field
`default-name` and import does not populate it. So a freshly imported port
plans as:

```
~ resource "routeros_interface_ethernet" "probe" {
    + factory_name = "ether2"
...
That would never converge — every plan proposes adding it again. It is an import
artefact rather than drift, and gets the same treatment as `block_access` in
Wave 3a:
...
Worth noting *because* it looks harmless: applying it to find out is an
in-place update against the WAN port and the LAN trunk.

#### MAC addresses are deliberately unmanaged

`mac_address` is writable, and on every port it equals the burned-in
`orig-mac-address`. Managing it buys nothing and costs something real: changing
ether1's MAC would make the BGW issue a new DHCP lease, and the WireGuard
port-forward targets the address on the current one.

**Verified that omitting it produces no drift** — this provider only diffs
attributes present in config, so an omitted optional field is left alone. That
is the opposite of the `client_id` behaviour recorded in Wave 3a, so it is
per-resource and worth testing rather than assuming either way.

It also means no SOPS file for this wave. The only identifying value a port
carries is the MAC; with that unmanaged, what remains is generic hardware
config, and plaintext keeps it reviewable.

#### What the ports actually are

| Port | | |
| --- | --- | --- |
| `ether1` | WAN | the only port not enslaved to the bridge |
| `ether2`–`ether4` | bridge slaves | **zero packets since boot** |
| `ether5` | bridge slave | the LAN trunk, and the only PoE-out port |
| `sfp1` | bridge slave | **zero packets since boot** |

Only ether1 and ether5 carry traffic. This is effectively a two-port router with
a switch hanging off ether5 — worth knowing before reasoning about the five
bridge ports Wave 5 imported as though they were all in use.

ether5 has `poe-out: auto-on`, so it will energise a PoE device if one is
attached.

#### The WAN client

Every address is computed — `192.168.100.66/24`, its gateway and the peer DNS
are assigned by the BGW and appear nowhere in config.

`use_peer_dns = false` is the load-bearing setting: the router resolves through
1.1.1.1 rather than the BGW, which is what keeps `time.cloudflare.com` and
everything else resolvable independently of the ISP.

`use_peer_ntp = true` is inert in practice — checked `/system/ntp/client` and it
carries only `time.cloudflare.com` with no dynamic servers, so nothing is being
injected from DHCP.

`add-default-route: yes` on the device **has no field in the provider schema**,
so tofu neither sets nor clears it. `default_route_distance` is the part it can
see. Something to remember before assuming this resource fully describes the
uplink.

### Wave 13b — fix what DHCP hands out for DNS, and adopt the resolver *(staged)*

Three DNS problems found while answering "can upstream DNS be encrypted?", and
the answer to that question turned out to be "mostly, already".

**AdGuard already does DoH.** Its upstream list is Cloudflare, Quad9 and AdGuard
over `https://.../dns-query`, with Quad9 unsecured as bootstrap and split-horizon
rules pointing the local zones back at `192.168.1.1:53`. Every client that uses
AdGuard was already encrypted. The gaps were the clients that do not.

#### The dead resolver

`192.168.1.3` is the Synology, and it was handed out as the DNS server for
**Servers (both `10.1.0.0/16` and `192.168.1.0/24`), Management and defconf**.

```
dig @192.168.1.3 example.com  ->  communications error: connection refused
```

ICMP port-unreachable rather than a timeout, so the host is up and nothing is
listening. It is also in the `Universal Accessible` address list commented
"NAS DNS", so this was intended and has since stopped working. Tested from the
Workstations VLAN; worth reconfirming from a Servers-VLAN host.

The cluster is unaffected -- the k3s nodes clearly do not take their resolver
from DHCP -- but any DHCP client on those three networks got a resolver that
refuses connections.

Each of those networks now points at **the router's own address on that network**,
which is already the `gateway` value:

| Network | was | now |
| --- | --- | --- |
| `10.1.0.0/16` Servers | `192.168.1.3` | `10.1.0.1` |
| `192.168.1.0/24` Servers | `192.168.1.3` | `192.168.1.1` |
| `192.168.99.0/24` mgmt | `192.168.1.3` | `192.168.99.1` |
| `192.168.88.0/24` defconf | `192.168.1.3` | `192.168.88.1` |

Safe because the input chain's only restriction is `drop all not coming from
LAN`, and every VLAN interface is in the `LAN` interface list -- so the router
answers DNS on each of those addresses. `allow-remote-requests` is already true.

#### Guest was on plaintext 1.1.1.1

`10.4.0.0/24` was handed `1.1.1.1` directly, bypassing AdGuard and resolving in
the clear. Now `192.168.1.5, 192.168.1.6`, the same as IoT and Workstations.

This works across VLANs *and survives a future default-drop* because of `f15`
`Allow Local Networks to Universal Accessible`, which sits above the IoT deny and
lists both AdGuard addresses. That is the same rule issue #1521 calls dead weight
under default-accept -- it is exactly what makes this safe once the drop exists.
Guest is in `Local Network`, so it is covered.

#### The resolver is now managed

`routeros_ip_dns` added to `dns.tf`. See the comment there for why this one is
declared rather than imported -- the provider refuses import outright, and the
plan was checked to confirm the unmanaged attributes come back as
`known after apply` rather than being reset.

`/ip/dns` was missing from the coverage audit altogether, so this closes a gap in
the sweep rather than a known exclusion.

#### DoH: tried properly, abandoned. Plaintext Quad9 is what runs

This took an outage to establish, so it is written out in full.

**Quad9 DoH cannot work on this board.** Enabling
`https://dns.quad9.net/dns-query` stopped recursive resolution immediately:

```
DoH server response not OK: 505: This server implements RFC 8484 - DNS Queries
over HTTP, and requires HTTP/2 in accordance with section 5.2 of the RFC.
```

Quad9 enforces HTTP/2; RouterOS's DoH client is HTTP/1.1. Confirmed off-device,
so it is not local misconfiguration and no certificate work fixes it:

| Endpoint | HTTP/1.1 | HTTP/2 |
| --- | --- | --- |
| `dns.quad9.net` | **505** | 200 |
| `dns.mullvad.net` | refuses ALPN | 200 |
| `doh.applied-privacy.net` | **505** | 200 |
| `dns.digitale-gesellschaft.ch` | **505** | 200 |
| `cloudflare-dns.com` | 200 | 200 |
| `dns.google` | 200 | 200 |
| `freedns.controld.com` | 200 | 200 |
| `doh.opendns.com` | 200 | 200 |
| `*.adguard-dns.com` | 200 | 200 |

**Firmware does not fix it.** RouterOS 7.23 added "HTTP/2 support to DoH on
ARM64 and x86/CHR devices". This is a hEX S on **mmips**, which is neither, and
no changelog through 7.24.2 mentions mmips. Installed 7.20.4, latest 7.24.2 --
worth upgrading, but not for this.

##### `servers` is NOT a fallback

The most important correction in this file. With `use_doh_server` set and
failing, RouterOS did **not** fall back to the `servers` list -- external
resolution stopped outright while the static table kept answering. That is what
made the outage look half-working: local names resolved, so the first instinct
was "DNS is fine". **DoH here fails closed.** Never enable it on the assumption
that a broken endpoint degrades gracefully.

Recovery is `/ip dns set use-doh-server="" verify-doh-cert=no` over winbox, which
is unaffected. Note `POST /rest/ip/dns/set` is the working REST path; `PATCH
/rest/ip/dns` silently does nothing and returns null.

##### ControlD worked, then flaked

`https://freedns.controld.com/p1` cleared every bar Quad9 failed: HTTP/1.1
accepted, chain verified, no 505s, ~50ms, established TLS sessions to
`76.76.2.11:443` and zero errors on day one.

It did not hold. Over the following days:
...
Resolution stayed fast between failures (6-15ms), so this presented as
intermittent flakiness rather than an outage -- harder to notice and more
annoying than a clean break.

**The cause is connection reuse, not the provider.** The server closes an idle
keepalive connection and RouterOS's HTTP/1.1 DoH client does not recover
cleanly. RouterOS 7.23 shipped fixes aimed exactly at this class:
...
All HTTP/2, all "ARM64 and x86/CHR devices" only. **So DoH on mmips is
intermittently unreliable whatever provider is chosen, and no firmware upgrade
in the 7.2x line changes that.** Treat DoH on this board as unavailable rather
than merely awkward.

##### What actually runs

`servers = ["9.9.9.9", "149.112.112.112"]`, plaintext 53, `use_doh_server = ""`.

Plaintext Quad9 keeps the malware blocklist and DNSSEC validation, which were
the goal; the encryption was the means. `9.9.9.10` / `149.112.112.10` is the
unfiltered pair if the blocklist ever gets in the way.

**Be precise about what DNSSEC buys here.** Validation happens at Quad9,
protecting Quad9 from lying authoritative servers. The Quad9-to-router hop is
plaintext and unvalidated, and RouterOS is a forwarder rather than a validating
resolver, so it does **not** protect against an on-path ISP. Only encryption to
the resolver would, and that is what is unavailable.

Scope, for perspective: this covers the router's own lookups and the VLANs
pointed at it. Every client behind AdGuard still resolves over DoH -- to Quad9
among others -- because AdGuard speaks HTTP/2. The k3s nodes point at
`192.168.1.1` via netplan, so they follow the router.

Tested 2026-09: the ISP is not intercepting port 53 (a query to TEST-NET-2 times
out rather than being answered) and not hijacking NXDOMAIN. That is observed
behaviour, not a property that has been secured.

##### The certificate work is left in place

The USERTrust ECC root imported for ControlD stays on the device. It is
harmless, and it saves redoing a manual step if DoH is ever worth revisiting.
Note it is a *different* anchor from the DigiCert Global Root G3 imported for
the Quad9 attempt -- **switching DoH provider means switching trust anchor**,
and `routeros_system_certificate` can only generate certificates, never import
PEM.

##### Pre-flight before enabling any DoH endpoint

`/tool fetch url="<endpoint>" check-certificate=yes mode=https dst-path=probe.txt`

This exercises name resolution, TLS, the trust anchor and the HTTP version from
the router itself, without touching DNS config. It would have caught the Quad9
failure before the outage, because `fetch` is HTTP/1.1 on this board too. Read
the result carefully:

- a certificate/TLS error means **stop**
- **505** means the provider requires HTTP/2 and is unusable here
- **400** is a **pass** -- a bare GET is not a valid DoH query, so the endpoint
  answering at the HTTP layer at all proves TLS and the chain are good

Note this needs the `test` policy, which the `tofu` user is denied; run it as
`admin`.

##### Verifying it is actually in use

Config alone proves nothing, and neither does resolution -- the static table
answers local names even when upstream is dead. The decisive check is live
connections:

```bash
/rest/ip/firewall/connection   -> established TCP to 76.76.2.11:443
/rest/log                      -> zero DoH error lines
```

Outbound `:53` connection count should be zero.

---

### Wave 14 — IPv6 *(done)*

`ipv6.tf` plus two maps in `locals.tf`. Thirty-three resources, all clean on the
first plan: 22 filter rules, 9 address-list entries, and the `ipv6_settings` and
`ipv6_neighbor_discovery` singletons. **276 resources across 35 types.**

Address-list entries import before the filter rules, since two of the rules
reference `bad_ipv6`.

Note the provider calls it **`routeros_ipv6_neighbor_discovery`**, not
`routeros_ipv6_nd`, which is what the REST path suggests.

#### Everything here is untouched defconf

All 22 rules and all 9 address-list entries are RouterOS stock. Nothing has been
hand-edited, which is a meaningful difference from the IPv4 firewall in Wave 4.

Plaintext rather than SOPS: every address is a well-known reserved range from
the stock ruleset and none of it is device-identifying. Encrypting a security
control for no benefit would only make it unreviewable.

#### What is NOT managed, and why there is nothing to manage

| Path | |
| --- | --- |
| `ipv6/address` | 9 entries, **all dynamic** (8 link-local) |
| `ipv6/route` | 9 entries, **all dynamic** |
| `ipv6/nd/prefix`, `ipv6/dhcp-client`, `ipv6/pool`, `ipv6/firewall/nat` | empty |
| `ipv6/neighbor` | 91 entries, runtime discovery rather than config |

So the wave is the firewall plus two settings singletons, and that is the whole
IPv6 configuration surface on this device.

#### Why bother, given it is inert

There is no GUA, so nothing routes over v6 and every address is link-local. But
`disable-ipv6` is false and `forward` is true. The day the ISP hands out a
prefix, this ruleset is the only thing in the way — and nobody would think to
review it at that moment, because nothing will have visibly changed. Importing
it now means a plan reports any drift.

#### Against issue #1521

The v6 forward chain **does** end in a drop (`v6f22`), where the v4 chain does
not. That looks like better segmentation and is not: the drop is
`in-interface-list: !LAN`, so LAN-to-LAN forwarding still falls through
accepted, exactly as in v4. IPv6 is not more segmented, just less hand-edited.

#### Ordering

Same constraint as Wave 4 — a tofu-CREATED rule lands at the bottom of its
chain, below the final drop. All 22 were imported, so tofu only ever updates in
place. Adding a rule means adding it on the router in the right position first,
then importing it.
...
**Provider surface:** 254 resource types. **Managed:** 276 resources across 35
...
- **`ip/service`** — a resource exists but cannot be imported; it reads back
  empty. See Wave 12. Disabling telnet, ftp and www is therefore a manual
  change, and worth doing.
- **`ip/neighbor/discovery-settings`** — a resource exists but its import
  requires an `.id` the endpoint does not return. See Wave 12.
